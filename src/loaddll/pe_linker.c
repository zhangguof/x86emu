...
bool pe_load_library(const char *filename, void **image, size_t *size)
{
    ...
    *size   = 0;
    ...
    if ((fd = open(filename, O_RDONLY)) < 0) {
        LOG_ERROR("failed to open pe library %s, %m", filename);
        goto error;
    }

    // Stat the file descriptor to determine filesize.
    if (fstat(fd, &buf) < 0) {
        LOG_ERROR("failed to stat the specified pe library %s, %m", filename);
        goto error;
    }
    ...
    *image = mmap(NULL, *size, PROT_READ, MAP_SHARED, fd, 0);
    ...
    if (*image == MAP_FAILED) {
        LOG_ERROR("failed to map library %s, %m", filename);
        goto error;
    }
...
//            DEBUG("Resetting ThreadInfo.ExceptionList");
//        }
